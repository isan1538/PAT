...
Added train/test mode split for proper generalization testing.
...
Physics-informed transformer model with train/test split:
python pat_training.py --M 50 --nu 0.1 --train_modes 1 2 --test_modes 3 4 5 \
    --alpha 1.0 --steps 5000 --weight_pde 0.1 --weight_bc 0.1 --weight_ic 0.1 \
    --save_path checkpoints/pat.pt
"""
...
def evaluate_model(model, nu, mode, device, nx=128, nt=64, M_eval=50):
    """
    Evaluate model by generating appropriate sparse context for the given mode.
    
    Args:
        model: PAT model
        nu: diffusivity
        mode: Fourier mode to evaluate (n=1,2,3,...)
        device: torch device
        nx, nt: evaluation grid resolution
    ...
    with torch.no_grad():
        ...
        u_sparse = exact_solution(
            xt_sparse[0, :, 0], xt_sparse[0, :, 1], nu, n=mode
        ...
        pred = model(u_sparse, xt_sparse, XT_eval)
...
def plot_training_progress(history, save_path):
    ...
    if len(history["eval_step"]) > 0:
        ...
        ax4.semilogy(eval_steps, eval_mses, "b-o", linewidth=2, markersize=4, label="Train MSE")
        if len(history.get("test_mse", [])) > 0:
            test_steps = to_numpy(history["test_step"])
            test_mses = to_numpy(history["test_mse"])
            ax4.semilogy(test_steps, test_mses, "r-o", linewidth=2, markersize=4, label="Test MSE")
    ...
    if len(history["eval_step"]) > 1:
        ...
        ax6.plot(eval_steps, eval_rels, "b-o", linewidth=2, markersize=4, label="Train RelErr")
        if len(history.get("test_rel", [])) > 0:
            test_steps = to_numpy(history["test_step"])
            test_rels = to_numpy(history["test_rel"])
            ax6.plot(test_steps, test_rels, "r-o", linewidth=2, markersize=4, label="Test RelErr")
        ax6.set_xlabel("Training Step")
        ax6.set_ylabel("Relative Error")
        ax6.set_title("Train vs Test Relative Error")
        ax6.legend()
...
def train_pat_sparse(args):
    ...
    print(f"\n{'='*80}")
    print("PAT - Sparse Reconstruction Training")
    print(f"{'='*80}")
    print(f"Device: {device}")
    print(f"Sparse points (M): {args.M}")
    print(f"Training modes: {args.train_modes}")
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % args.eval_every == 0 or step == args.steps:
            eval_results = evaluate_model(
                model, args.nu, args.train_modes[0], device, M_eval=args.M
            )

            print(
                f"       EVAL (seen mode {args.train_modes[0]}) → MSE={eval_results['mse']:.3e} "
            ...
            test_results = evaluate_model(
                model, args.nu, args.test_modes[0], device, M_eval=args.M
            )
            
            print(
                f"       TEST (unseen mode {args.test_modes[0]}) → MSE={test_results['mse']:.3e} "
    ...
    for mode in args.train_modes:
        eval_seen = evaluate_model(
            model, args.nu, mode, device, nx=256, nt=128, M_eval=args.M
        )
    ...
    for mode in args.test_modes:
        eval_unseen = evaluate_model(
            model, args.nu, mode, device, nx=256, nt=128, M_eval=args.M
        )
        print(f"Mode {mode}: MSE={eval_unseen['mse']:.4e}, MAE={eval_unseen['mae']:.4e}, "
              f"RelErr={eval_unseen['rel_error']:.4e}")
    
    # Use first training mode for plots
    final_eval = evaluate_model(
        model, args.nu, args.train_modes[0], device, nx=256, nt=128, M_eval=args.M
    )
    ...
    if args.plot:
        ...
        plot_training_progress(history, "pat_training_progress.png")
        plot_solution_comparison(final_eval, "pat_seen_mode.png")
        
        # Also plot unseen mode
        unseen_eval = evaluate_model(
            model, args.nu, args.test_modes[0], device, nx=256, nt=128, M_eval=args.M
        )
        plot_solution_comparison(unseen_eval, "pat_unseen_mode.png")
...
def main():
    ...
    ap.add_argument("--train_modes", type=int, nargs="+", default=[1, 2], help="Training sine modes")
    ap.add_argument("--test_modes", type=int, nargs="+", default=[3, 4, 5], help="Test sine modes")
