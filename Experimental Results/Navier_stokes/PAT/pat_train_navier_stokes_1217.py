...
################################################################
python pat_train_navier_stokes_improved.py --mode pure --data_path ./cylinder_nektar_wake.mat \
  --device cuda --seed 0 --Re 100 --eval_t_index 100 --n_train 2500 \
  --n_colloc 5000 --w_data 1.0 --w_p 1.0 --w_pde 0.0 --w_cont 0.0 \
  --steps 1000 --lr 3e-4 --save_path checkpoints/pat_pure.pt \
  --out_dir checkpoints/pat_pure  

################################################################
  Method: Physics-Aware Transformer with PDE constraints and diffusion bias
################################################################
python pat_train_navier_stokes.py --mode physics --data_path ./cylinder_nektar_wake.mat \
  --device cuda --seed 0 --Re 100 --eval_t_index 100 --n_train 2500 \
  --n_colloc 1000 --w_data 1.0 --w_p 1.0 --w_pde 1.0 --w_cont 1.0 \
  --alpha 1.0 --steps 1000 --lr 3e-4 --save_path checkpoints/pat_physics.pt \
...
################################################################
python pat_train_navier_stokes.py --mode streamfunction \
  --data_path ./cylinder_nektar_wake.mat --device cuda --seed 0 \
  --Re 100 --eval_t_index 100 --n_train 2500 --n_colloc 5000 \
  --w_data 1.0 --w_p 1.0 --w_pde 1.0 --w_cont 0.0 --alpha 1.0 \
  --steps 1000 --lr 3e-4 --save_path checkpoints/pat_streamfunction.pt \
...
if __name__ == "__main__":
    ...
    if save_path:
        checkpoint = torch.load(save_path.replace(".pt", "_best.pt"))
        checkpoint['metrics'] = metrics
        torch.save(checkpoint, save_path.replace(".pt", "_best.pt"))
        print(f"Saved metrics to checkpoint")