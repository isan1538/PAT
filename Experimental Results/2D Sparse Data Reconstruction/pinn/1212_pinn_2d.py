...
class PINN2D(nn.Module):
    ...
    def forward(self, xyt):
        xyt_norm = 2 * xyt - 1
        h = self.activation(self.input_layer(xyt_norm))
...
def evaluate_model_2d(model, nu, mode, device, nx=128, ny=128):
    ...
    return {'mse': mse, 'mae': mae, 'rel_error': rel_error, 'max_error': max_error, 
            'prediction': u_pred.cpu(), 'ground_truth': u_true.cpu()}


def plot_results_2d(history, eval_results, save_path='pinn_2d_results.png', mode=None):
    def to_numpy(data):
    ...
    ax5.set_xlabel('x'); ax5.set_ylabel('y'); ax5.set_title('PINN Prediction'); plt.colorbar(im, ax=ax5)
    ...
    if mode:
        fig.suptitle(f'PINN 2D - Mode ({mode[0]},{mode[1]})', fontsize=14, y=0.995)
    
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()


def train_pinn_2d(args):
    ...
    model = PINN2D(hidden_dim=args.hidden_dim, num_layers=args.num_layers, activation=args.activation).to(device)
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % args.print_every == 0 or step == 1:
            ...
            print(f"[{step:05d}] loss={stats['loss']:.3e} | data={stats['data']:.3e} pde={stats['pde']:.3e} "
                  f"bc={stats['bc']:.3e} ic={stats['ic']:.3e} | lr={lr_current:.2e}")
    ...
    if args.plot:
        ...
        plot_results_2d(history, final_eval, 'pinn_2d_seen.png', train_modes[0])
        unseen_eval = evaluate_model_2d(model, args.nu, test_modes[0], device)
        plot_results_2d(history, unseen_eval, 'pinn_2d_unseen.png', test_modes[0])
